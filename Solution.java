...
public class Challenge {
   ...
   * message is shifted to the right of the alphabet. If the total number of shifts exceeds the
   * number of letters in the alphabet, then it continues counting from the first alphabet character
   * again, and so on, until the total number of shifts is reached.
   *
   * <p>The positions of the punctuation marks, spaces and the letter case is as in the original
   * message.
   *
   * @return A string, representing the ecnoded message.
   */
  private static String caesarCipher_fromPlainText_toCipherText(
      String plain_text, int total_shifts) {
  ...
  private static char findCipherChar(char char_text, int total_shifts) {
    ...
    // The method is called only if the char is already confirmed as a letter.
    // Therefore, if the code point is greater than 96, then it is lowercase.
